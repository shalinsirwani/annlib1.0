...
void Layer::setVal(int i, double v , bool ad) {
  this->neurons.at(i)->setVal(v , ad);
}
...
Layer::Layer(int size, int activationType , bool ad) {
  this->size = size;
  for(int i = 0; i < size; i++) {
    Neuron *n = new Neuron(0.000000000, activationType , ad);
    this->neurons.push_back(n);
  }
}
